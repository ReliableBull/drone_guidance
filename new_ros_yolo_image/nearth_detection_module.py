...
# point_msg.position.x = -1
# point_msg.position.y = -1
# point_msg.position.z = -1
 ...
 # log some info about the image topic
        ...
        if flag == 1:
            for bbox, class_id, seg, score in zip(bboxes, classes, segmentations, scores):
                ...
                point_msg.pose.position.x = int(centerX)
                point_msg.pose.position.y = int(centerY)
                point_msg.pose.position.z = width
...
sub_image = rospy.Subscriber("/iris_depth_camera_0/camera/rgb/image_raw", Image, image_callback)
 # "Gazebo/link_states" 토픽을 구독할 Subscriber 생성
rospy.Subscriber("/gazebo/link_states", LinkStates, link_states_callback)

# publisher
pub = rospy.Publisher('center_of_image', PoseStamped, queue_size=10)
