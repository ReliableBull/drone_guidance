...
def link_states_callback(msg):
    if len(msg.pose) > 25:
        pose1 = msg.pose[23]  # 23번째 링크의 포즈 정보
        pose2 = msg.pose[25]  # 25번째 링크의 포즈 정보
...
def image_callback(img_msg):
    ...
    if flag == 1:
        for bbox, class_id, seg, score in zip(bboxes, classes, segmentations, scores):
            ...
            if image_cnt % 20 == 0:
                output_filename = f'output_{jpg_index}.jpg'

                output_file_path = os.path.join("vali_image", output_filename)
                ...
                output_txt_filename = f'output_{jpg_index}.txt'

                output_txt_file_path = os.path.join("vali_image", output_txt_filename)

                with open(output_txt_file_path, 'w') as file:
                    file.write(f'Drone {score} {x} {y} {x2} {y2}\n')
                    # file.write(f'{y}\n')
                    # file.write(f'{x2}\n')
                    # file.write(f'{y2}\n')
...
rospy.init_node('image info', anonymous=True)
