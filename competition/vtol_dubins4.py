...
# 특정 지점 계산 함수
def compute_point_from(lat, lon, distance, bearing):
...
# 두 번째 웨이포인트를 지나서 왼쪽 상단에 위치한 원 회전 시작 지점 계산
left_top_point = compute_point_from(waypoints[1][0], waypoints[1][1], 50, 225)

# 두 번째 웨이포인트에서 왼쪽 상단의 특정 지점으로 이동
start = gps_to_rad(*waypoints[1])
end = gps_to_rad(*left_top_point)
start_with_angle = (start[0], start[1], 0)
end_with_angle = (end[0], end[1], 0)
path2 = get_dubins_path(start_with_angle, end_with_angle, turning_radius)
paths.extend(path2[0])

# 왼쪽 상단의 특정 지점에서 두 번째 웨이포인트로 돌아오기
start = gps_to_rad(*left_top_point)
end = gps_to_rad(*waypoints[1])
start_with_angle = (start[0], start[1], math.pi)
end_with_angle = (end[0], end[1], 0)
path3 = get_dubins_path(start_with_angle, end_with_angle, turning_radius)
paths.extend(path3[0])

# 두 번째 웨이포인트에서 세 번째 웨이포인트로 직선 비행
start = gps_to_rad(*waypoints[1])
end = gps_to_rad(*waypoints[2])
start_with_angle = (start[0], start[1], 0)
end_with_angle = (end[0], end[1], 0)
path4 = get_dubins_path(start_with_angle, end_with_angle, turning_radius)
paths.extend(path4[0])
...
waypoint_tolerance = 15.0  # meters
