...
waypoints = [
    ...
    (47.4081627, 8.5525792),  # 중간 좌표
    (47.4079917, 8.5525833), 
    (47.4079660, 8.5558650),   # 목표 좌표
   (47.4079660, 8.5558650), # 목표 좌표
]

# Dubins 경로 생성
turning_radius = 0.01  # 약 200m 반경
paths = []

# 모든 웨이포인트 쌍에 대해 Dubins 경로 생성
for i in range(len(waypoints) - 1):
    start = gps_to_rad(*waypoints[i])
    end = gps_to_rad(*waypoints[i + 1])
    start_with_angle = (start[0], start[1], 0)
    end_with_angle = (end[0], end[1], math.pi)  # 각도 수정 (필요에 따라 수정)
    path = get_dubins_path(start_with_angle, end_with_angle, turning_radius)
    paths.extend(path[0])
